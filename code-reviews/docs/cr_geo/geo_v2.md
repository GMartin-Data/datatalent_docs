...
import requests
from geo.config import BASE_URL, RESOURCES  # --- [5] BUCKET_NAME et DATASET_ID retirés :
...
5. **`BUCKET_NAME` et `DATASET_ID` supprimés** de `config.py` et de l'import dans `ingest.py`.